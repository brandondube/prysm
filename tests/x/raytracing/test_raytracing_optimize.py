...
_glass = materials.ConstantMaterial('N-BK7', 1.5168)
...
def make_singlet(image_gap=96.0, fields=(0,), wavelengths=(0.55,),
    ...
    (lens.add(Conic(1 / 102.0, 0.0), thickness=6.0, material=_glass,
         ...
         .add(Plane(), typ='eval', semidiameter=12.0))
...
def test_constrained_optimize_hits_efl():
    ...
    with warnings.catch_warnings():
        ...
        sys_.optimize('spot', constraints=[EFL(target=100.0)],
                      maxiter=30, damping=1e-8)
    assert effective_focal_length(sys_, wvl=0.55) == pytest.approx(
        100.0, rel=1e-5)
...
def test_fanout_explicit_wavelengths_weight_uniformly():
    ...
    prob = sys_.problem('spot', wavelengths=[0.5])
...
def test_fanout_mixes_string_subclass_instance_and_callable():
    ...
    prob = sys_.problem(['spot', _FanMerit, inst, my_merit])
...
def test_constraint_routing_targets_and_bounds():
    sys_ = make_singlet()
    prob = sys_.problem('spot', constraints=[
        EFL(target=100.0),
        TotalTrack(max=110.0),
        Thickness(1, min=5.0, max=120.0),
    ])
...
def test_total_track_and_thickness_match_hand_sums():
    ...
    assert Thickness(1)(sys_, cache) == pytest.approx(96.0)


def test_total_track_skips_object_row():
    lens = LensData()
    (lens.add(Plane(), typ='eval', thickness=np.inf)
         .add(Conic(1 / 102.0, 0.0), thickness=6.0, material=_glass)
...
def test_bundle_relaunches_as_the_design_moves():
    ...
    sys_ = make_singlet(fields=(3.0,), stop_index=1)
