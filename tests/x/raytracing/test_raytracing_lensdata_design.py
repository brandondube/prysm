...
def test_problem_residuals_track_the_free_vector():
    ...
    assert r0[0] == pytest.approx(effective_focal_length(ld.to_surfaces(), wvl=wvl) - target)


def test_lensdata_efl_optimization_converges():
    ld = make_singlet()
    ld.opt.vary('curvature', surfaces=1)  # one DOF, one operand -> well posed
    wvl = ld.wavelength()
    target = 80.0
    ...
    assert res.success
    assert effective_focal_length(ld.to_surfaces(), wvl=wvl) == pytest.approx(target, rel=1e-6)
...
def test_lensdata_thickness_and_curvature_jointly_varied():
    ...
    ld = make_singlet()
    # conic-2 curvature + conic-1 gap (rows 1 and 2; rows[0] is OBJECT)
    ld.opt.vary('curvature', surfaces=2).vary('thickness', surfaces=1)
    wvl = ld.wavelength()
    ...
    assert res.success
    assert effective_focal_length(ld.to_surfaces(), wvl=wvl) == pytest.approx(90.0, rel=1e-5)
...
def test_jacobian_restores_free_vector():
    ...
    x0 = prob.x0()
    prob.jacobian(x0)
    np.testing.assert_allclose(ld.opt.pack(), x0)
...
def _fresh_cache(prob):
    ...
    return _TraceCache(prob.system)
