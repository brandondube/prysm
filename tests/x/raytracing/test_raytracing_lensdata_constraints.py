...
def test_selector_int():
    ld = make_triplet()
    ld.opt.vary('curvature', surfaces=2)
    assert len(ld.opt.pack()) == 1


def test_selector_list():
    ld = make_triplet()
    ld.opt.vary('curvature', surfaces=[1, 3, 5])   # three of the six conics
    assert len(ld.opt.pack()) == 3


def test_selector_slice():
    ld = make_triplet()
    ld.opt.vary('curvature', surfaces=slice(1, 4))
    assert len(ld.opt.pack()) == 3


def test_selector_all_skips_planes():
    ld = make_triplet()
    ld.opt.vary('curvature', surfaces='all')
    # 6 conics have curvature; the eval plane does not
    assert len(ld.opt.pack()) == 6


def test_selector_negative_index():
    ld = make_triplet()
    ld.opt.vary('curvature', surfaces=-2)  # last conic
    np.testing.assert_allclose(ld.opt.pack(), [1.0 / -50.0])
...
def test_absolute_thickness_bounds():
    ld = make_triplet()
    ld.opt.vary('thickness', surfaces=[1, 2])
    ld.opt.constrain('thickness', lo=0.0, hi=20.0, surfaces=[1, 2])
    lo, hi = ld.opt.bounds()
...
def test_absolute_one_sided_bound_leaves_other_infinite():
    ...
    ld.opt.constrain('thickness', lo=0.0, surfaces=1)
    lo, hi = ld.opt.bounds()
    assert lo[0] == 0.0
...
def test_relative_curvature_bound_is_pct_of_curvature():
    ld = make_triplet()
    ld.opt.vary('curvature', surfaces=1)          # first conic, radius 50
...
def test_relative_radius_bound_is_pct_of_radius():
    ld = make_triplet()
    ld.opt.vary('radius', surfaces=1)             # first conic, radius 50
...
def test_relative_radius_bound_orders_negative_nominal():
    ld = make_triplet()
    ld.opt.vary('radius', surfaces=2)  # radius -80 (rows[0] is OBJECT)
    ld.opt.constrain('radius', relative=0.1, surfaces=2)
    lo, hi = ld.opt.bounds()
    assert lo[0] < hi[0]
    # radius window [-88, -72]
...
def test_constrain_missing_category_is_silent_no_op():
    ...
    ld.opt.constrain('radius', relative=0.1, surfaces='all')
    assert len(ld.opt.bounds()[0]) == 6


def test_constrain_requires_a_bound_spec():
    ld = make_triplet()
    with pytest.raises(ValueError):
        ld.opt.constrain('thickness', surfaces=0)


def test_bounds_only_returned_for_free_slots():
    ld = make_triplet()
    ld.opt.constrain('thickness', lo=0.0, hi=10.0, surfaces='all')  # set, not freed
