...
The optimization tools now include governed runners and convergence plotting.
The :func:`~prysm.x.optym.optimizers.run_until` runner pairs any step-based
optimizer with reusable governors such as
...
:class:`~prysm.x.optym.governors.MaxEvaluations`, and
:class:`~prysm.x.optym.governors.ConstraintTolerance`.  The
:func:`~prysm.x.optym.plotting.plot_convergence` helper plots objective value,
gradient norm, and active bounded-variable counts from the resulting step
records.  See :doc:`../how-tos/Advanced Optimization with Problem and Governors`
for a complete example using :class:`~prysm.x.optym.problem.Problem`,
L-BFGS-B bounds, governors, and convergence plotting.

Benchmark sample problems are available as ready-to-use :code:`(f, g)`
callables for optimizer examples, smoke tests, and comparisons:

* :func:`~prysm.x.optym.sample_problems.sphere`
* :func:`~prysm.x.optym.sample_problems.rosenbrock`
* :func:`~prysm.x.optym.sample_problems.rastrigin`
* :func:`~prysm.x.optym.sample_problems.himmelblau`
...
All have full support for GPUs and 32-bit numbers, except for LBFGSB which is
CPU-only and double precision only.  The public LBFGSB alias dispatches to the
SciPy Fortran or C L-BFGS-B driver as appropriate for the installed SciPy
version; :class:`~prysm.x.optym._lbfgsb.F77LBFGSB` and
:class:`~prysm.x.optym._lbfgsb.CLBFGSB` remain available when a specific driver
is desired.

Problem definition, line search, and operators

* :class:`~prysm.x.optym.problem.Problem`
* :func:`~prysm.x.optym.problem.as_problem`
...
Some work has been done on computing OPD, improving ray aiming, and surfaces
that are more generally off axis in each of x and y. This module is still a ways
away from prime time. An enormous improvement in capability will come in the
next update, with improved documentation brought up alongside it.
