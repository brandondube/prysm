...
compatibility and API stability will be guaranteed. In preparation for this, the
library has been split into mature code such as coordinates, geometry,
...
In general, there may be an additional batch of breaking changes in the 0.22 =>
1.0 update.
...
parameters, perform phase retrieval, etc.  Models made with this semi-manual
backpropagation code are typically over an order of magnitude faster than other
packages using machine learning libraries such as Jax or Pytorch to do the
backpropagation, and allow certain operations like array insertion and cropping
that are forbidden by other libraries.
...
linear algebra routines, alongside :attr:`prysm.mathops.np` and
:attr:`prysm.mathops.fft`.  :func:`~prysm.mathops.array_to_true_numpy` also
passes scalar values through without forcing array materialization, which
reduces overhead in optimizer and line-search bookkeeping.
...
The thin lens module has grown into a more general collection of first-order
optics formulas.  New helpers cover focal power, conjugates, pupil diameter,
and common thick-lens reductions:

* :func:`~prysm.thinlens.object_image_to_efl` computes focal length from a
  pair of object and image conjugates
...
* :func:`~prysm.thinlens.singlet_power`,
  :func:`~prysm.thinlens.singlet_bfl`, and
  :func:`~prysm.thinlens.singlet_ffl` provide first-order properties of a
  thick singlet

* :func:`~prysm.thinlens.twolens_power`,
  :func:`~prysm.thinlens.twolens_ffl`, and
  :func:`~prysm.thinlens.twolens_separation` provide additional two-thin-lens
  formulas
...
length of the two-lens system.  :func:`~prysm.thinlens.singlet_efl` now uses
...
new additions is :code:`x/optym`, a package for optimization with several cost
functions, activation functions, and gradient-based optimizers

x/optym
-------

Optym is a new optimization experimental package.  It provides numerous optimizers, as well as mechanisms for enforcing intelligent stopping conditions on running optimizers, and a different way of representing an optimization problem that allows for optimizers to only compute what they need to compute at a given time.  This means the interface is very different to :code:`scipy.optimize`.  It is the authors' belief that this interface is superior.  Independent of the interface, all optimizers support arithmetic at any precision, and can run on CPU or GPU, as well as any of the interchangeable backends.  This gives optym a wide open domain of applicability, including extremely rapid optimization of very high dimensional problems entirely on GPU, such as coronagraph mask design.

The basic building blocks are the user providing a minimal function :code:`cost(x) -> (f, g)` where f is the scalar cost function, and g is d/dx of f.  This can be provided as a more formal :class:`~prysm.x.optym.problem.Problem` object which can be more computationally efficient with certain optimizers.  This is driven to the minimum value of f using an optimizer.  Optimizers have a simple API: the user calls :code:`.step() -> (x, f, g)` to iterate one step, returning the value of the parameter vector, cost, and gradient *before* the step.  The new x vector is then accessible at :code:`optimizer.x`.  In most cases, you will not step the optimizer directly; instead calling :func:`~prysm.x.optym.optimizers.run_until` to run until a stopping condition is met, or :func:`~prysm.x.optym.optimizers.run_for` to run for a fixed number of steps.  :code:`run_until` can be given any :class:`~prysm.x.optym.governor.Governor` to define one or more stopping condition.  When multiple governors are given, the first of them to call stop will end iteration.
...
A stronge Wolfe conditions line searcher is available via :func:`~prysm.x.optym.linesearch.ls_strong_wolfe`.
...
All optimizers run on a :class:`~prysm.x.optym.problem.Problem`, or :code:`def cost(x) -> (f, g)`, which is automatically converted into a problem.  Run the optimizer by calling step in a loop, or run_until/run_for.
...
* :class:`~prysm.x.optym.governors.MaxIterations`
* :class:`~prysm.x.optym.governors.FunctionTolerance`
* :class:`~prysm.x.optym.governors.GradientTolerance`
* :class:`~prysm.x.optym.governors.StepTolerance`
* :class:`~prysm.x.optym.governors.MaxEvaluations`
* :class:`~prysm.x.optym.governors.ConstraintTolerance`.
...
* :class:`~prysm.x.optym.sample_problems.Sphere` or :func:`~prysm.x.optym.sample_problems.sphere`
* :class:`~prysm.x.optym.sample_problems.Rosenbrock` or :func:`~prysm.x.optym.sample_problems.rosenbrock`
* :class:`~prysm.x.optym.sample_problems.Rastrigin` or :func:`~prysm.x.optym.sample_problems.rastrigin`
* :class:`~prysm.x.optym.sample_problems.Himmelblau` or :func:`~prysm.x.optym.sample_problems.himmelblau`
...
* :class:`~prysm.x.optym.activation.GumbelSoftmax`
* :class:`~prysm.x.optym.activation.DiscreteEncoder`
