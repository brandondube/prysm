...
functions.  This created significant complexity with backpropagation.  The API
now requires two lines of code to do the equivalent, but usage is significantly
simplified.  The previous usage looked like:
...
that are more generally off axis in each of x and y.  This module is still a
ways away from prime time.
