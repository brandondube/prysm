...
def read_zygo_dat(file, multi_intensity_action='first'):
    ...
    multi_intensity_action : str, {'avg', 'first', 'last'}
        action to take when handling multiple intensity frames
    ...
    Notes
    -----
    Zygo stores rows in image order.  Returned phase and intensity arrays are
    flipped vertically so their y coordinates increase upward in prysm.
...
def write_zygo_dat(file, phase, dx, wavelength=0.6328, intensity=None):
    ...
    Notes
    -----
    Input arrays use prysm's upward-increasing y convention and are flipped
    vertically for Zygo's image-order storage.

    """
    if intensity is not None:
        raise NotImplementedError('writing DAT files with intensity is not supported')
    ...
    sf_nm = sf_m
    encoded = np.where(mask, 0, phase) / 1e9 * (1 / sf_nm)
    im = encoded.astype(np.int32)
    im[mask] = ZYGO_INVALID_PHASE

    dt = np.dtype(np.int32).newbyteorder('>')
    bufphs = im.astype(dt).tobytes(order='C')
    close_file = not hasattr(file, 'write')
    if close_file:
        file = open(file, 'wb')
...
def write_codev_gridint(array, filename, comment='CV GRD generated by prysm', typ='SUR', nnb=False):
    ...
    -----
    Input arrays use prysm's upward-increasing y convention and are flipped
    vertically for Code V's image-order storage.
...
def read_codev_gridint(file):
    ...
    Notes
    -----
    Code V stores rows in image order.  Returned arrays are flipped vertically
    so their y coordinates increase upward in prysm.
    ...
    if ssz is None:
        raise ValueError('CV INT header did not contain SSZ')
    if meaning is None:
        raise ValueError('CV INT header did not contain SUR or WFR')
