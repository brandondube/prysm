...
from .core import BaseMaterial, MaterialRangeError, MissingKError, _user_page_info
...
def _validate_samples(wavelengths, n, k, sigma_n, sigma_k):
    ...
    if np.any(wavelengths <= 0):
        raise ValueError('wavelengths must be positive microns')
...
def _coerce_temperature_grid(grid, temperatures, wavelengths, label):
    if grid is None:
        return None
...
def _interp_grid(wavelengths, temperatures, grid, wvl, temp, extrapolate):
    ...
    out = np.empty(flat_w.shape, dtype=grid.dtype)
    for idx, (w, t) in enumerate(zip(flat_w, flat_t)):
        by_temperature = np.array([
            _linear_interp(w, wavelengths, row, extrapolate)
            for row in grid
        ], dtype=out.dtype)
        out[idx] = _linear_interp(t, temperatures, by_temperature, extrapolate)
    return out.reshape(wvl_b.shape)
...
class TemperatureGridMaterial(BaseMaterial):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        n_grid = _coerce_temperature_grid(n, temperatures, wavelengths, 'n')
        n_grid = n_grid[t_order][:, w_order]
        k_grid = _coerce_temperature_grid(k, temperatures, wavelengths, 'k')
        if k_grid is not None:
            k_grid = k_grid[t_order][:, w_order]
        dn_dlambda_grid = _coerce_temperature_grid(
            dn_dlambda, temperatures, wavelengths, 'dn_dlambda'
        )
        if dn_dlambda_grid is not None:
            dn_dlambda_grid = dn_dlambda_grid[t_order][:, w_order]
        dn_dT_grid = _coerce_temperature_grid(dn_dT, temperatures, wavelengths, 'dn_dT')
        if dn_dT_grid is not None:
            dn_dT_grid = dn_dT_grid[t_order][:, w_order]
        sigma_n_grid = _coerce_temperature_grid(sigma_n, temperatures, wavelengths, 'sigma_n')
